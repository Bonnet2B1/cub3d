...
void	put_wall(t_game *data, t_ray *ray, int ray_idx)
{
	if (ray->side == 'N')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->north_img);
	else if (ray->side == 'S')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->south_img);
	else if (ray->side == 'E')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->east_img);
	else if (ray->side == 'W')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->west_img);
}
...
void	put_spe(t_game *data, t_ray *ray, int ray_idx)
{
	if (ray->side == 'N' || ray->side == 'S')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->pillar_img);
	if (ray->side == 'W' || ray->side == 'E')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->peacock_img);
}
...
void	put_mod_one(t_game *data, t_ray *ray, int ray_idx)
{
	if (ray->side == 'N' || ray->side == 'S')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->sarcophagus_img);
	else if (ray->side == 'E')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->anubis_chill_img);
	else if (ray->side == 'W')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->basic_wall_img);
}
...
void	put_mod_two(t_game *data, t_ray *ray, int ray_idx)
{
	if (ray->side == 'N' || ray->side == 'S')
	{
		if (data->assets->frame < ANIMATION_SPEED / 2)
			put_line(data, data->assets->textures_mask,
				ray, ray_idx, data->assets->mummy_left_img);
		else
			put_line(data, data->assets->textures_mask,
				ray, ray_idx, data->assets->mummy_right_img);
	}
	else if (ray->side == 'E')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->anubis_not_happy_img);
	else if (ray->side == 'W')
		put_line(data, data->assets->textures_mask,
			ray, ray_idx, data->assets->wall_w_hole_img);
}
